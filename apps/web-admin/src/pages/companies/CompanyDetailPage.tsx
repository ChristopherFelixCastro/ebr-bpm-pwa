...
export const CompanyDetailPage = () => {
  ...
  const canEdit = isGlobal || currentUser?.roleCode === 'COMPANY_ADMIN'
  ...
  return <Stack spacing={3}>
    ...
    <Card sx={{ border: '1px solid #E2E8F0' }}><CardContent><Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}><Typography variant="h6" sx={{ fontWeight: 700 }}>Establecimientos</Typography>{canEdit && company.status === 'ACTIVE' && <Button startIcon={<AddLocationAltIcon />} onClick={() => setEstablishmentOpen(true)}>Crear establecimiento</Button>}</Box><Divider sx={{ mb: 2 }} />
      {establishments.length === 0 ? <Alert severity="info">No existen establecimientos registrados.</Alert> : <Stack spacing={1}>{establishments.map((item) => <Button key={item.id} onClick={() => navigate(`/establishments/${item.id}`)} sx={{ justifyContent: 'space-between', border: '1px solid #E2E8F0', p: 1.5 }}><span>{item.name}</span><Chip size="small" label={item.status} color={item.status === 'ACTIVE' ? 'success' : 'default'} /></Button>)}</Stack>}
    </CardContent></Card>
