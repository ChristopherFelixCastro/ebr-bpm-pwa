...
import { isRequestReadOnly, submitPreconditionError } from '../../routes/requestRules'
...
export const RequestDetailPage = () => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      navigate(`/requests/${id}`, { replace: true, state: { caseId: result.caseId } })
      await load()
  ...
  return <Stack spacing={3}>
    ...
    <Card sx={{ border: '1px solid #E2E8F0' }}><CardContent><Typography variant="h6" sx={{ fontWeight: 700 }}>Datos de solicitud</Typography><Divider sx={{ my: 2 }} /><Typography><strong>Establecimiento:</strong> {request.establishmentName || request.establishmentId}</Typography><Typography><strong>Motivo:</strong> {request.reason}</Typography><Typography><strong>Observaciones:</strong> {request.observations || '—'}</Typography>{draft && <Button sx={{ mt: 2 }} variant="outlined" onClick={() => navigate(`/requests/${id}/edit`)}>Editar borrador</Button>}</CardContent></Card>

    <Card sx={{ border: '1px solid #E2E8F0' }}><CardContent><Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}><Typography variant="h6" sx={{ fontWeight: 700 }}>Contactos de la solicitud</Typography>{draft && <Button startIcon={<AddIcon />} onClick={() => setContactOpen(true)}>Vincular</Button>}</Box>{activeContacts.length === 0 ? <Alert severity="warning">Debe vincular al menos un contacto.</Alert> : <Stack spacing={1}>{activeContacts.map((contact) => <Box key={contact.id} sx={{ p: 1.5, border: '1px solid #E2E8F0', borderRadius: 2, display: 'flex', justifyContent: 'space-between', gap: 2 }}><Box><Typography sx={{ fontWeight: 700 }}>{contact.fullName}</Typography><Typography variant="body2" color="text.secondary">{contact.relationshipType} · versión {contact.version}</Typography></Box><Stack direction="row" spacing={1}>{contact.isPrimary && <Chip size="small" color="primary" label="Primario" />}{draft && <Button size="small" color="error" onClick={() => void removeContact(contact.id, contact.version)}>Retirar</Button>}</Stack></Box>)}</Stack>}</CardContent></Card>

    <Card sx={{ border: '1px solid #E2E8F0' }}><CardContent><Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 2, flexWrap: 'wrap', mb: 2 }}><Typography variant="h6" sx={{ fontWeight: 700 }}>Documentos privados</Typography>{draft && <Stack direction="row" spacing={1}><TextField select size="small" value={documentType} onChange={(event) => setDocumentType(event.target.value as DocumentType)}><MenuItem value="AUTHORIZATION_LETTER">Carta de autorización</MenuItem><MenuItem value="SUPPORTING_DOCUMENT">Documento de soporte</MenuItem></TextField><Button component="label" startIcon={<CloudUploadIcon />} variant="outlined">Cargar<input ref={fileInput} hidden type="file" accept="application/pdf,image/jpeg,image/png" onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file) }} /></Button></Stack>}</Box>
      {documents.length === 0 ? <Alert severity="warning">No existen documentos.</Alert> : <Stack spacing={1}>{documents.map((document) => <Box key={document.id} sx={{ p: 1.5, border: '1px solid #E2E8F0', borderRadius: 2 }}><Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 2, flexWrap: 'wrap' }}><Box><Typography sx={{ fontWeight: 700 }}>{document.fileName}</Typography><Typography variant="body2" color="text.secondary">{document.documentType} · {formatBytes(document.sizeBytes)} · versión {document.version}</Typography></Box><Chip size="small" label={document.status} color={document.status === 'VALID' ? 'success' : document.status === 'REJECTED' ? 'error' : 'default'} /></Box><Stack direction="row" spacing={1} sx={{ mt: 1 }}><Button size="small" startIcon={<DownloadIcon />} onClick={() => void download(document.id)}>Descarga temporal</Button>{draft && isGlobal && document.status === 'PENDING' && <><Button size="small" color="success" onClick={() => void documentAction(document, 'validate')}>Validar</Button><Button size="small" color="error" onClick={() => setRejecting(document)}>Rechazar</Button></>}{draft && isGlobal && document.status === 'REJECTED' && <Button size="small" color="warning" onClick={() => void documentAction(document, 'archive')}>Archivar</Button>}</Stack>{document.rejectionReason && <Alert severity="error" sx={{ mt: 1 }}>{document.rejectionReason}</Alert>}</Box>)}</Stack>}
    </CardContent></Card>

    {draft && <Button size="large" variant="contained" startIcon={<SendIcon />} onClick={() => setSubmitOpen(true)}>Enviar solicitud</Button>}
