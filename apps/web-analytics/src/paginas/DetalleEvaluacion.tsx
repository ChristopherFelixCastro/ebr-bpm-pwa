...
export function DetalleEvaluacion() {
  ...
  const download = async () => { if (!official) return; setProcessing('Descarga'); try { const result = await core.reportDownloadUrl(id, official.id); window.open(result.data.signedUrl, '_blank', 'noopener,noreferrer'); } catch (cause) { setError(cause instanceof Error ? cause.message : 'No se pudo generar la descarga temporal.'); } finally { setProcessing(''); } };
