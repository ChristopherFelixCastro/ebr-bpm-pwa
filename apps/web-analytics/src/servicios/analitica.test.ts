...
describe('ServicioAnaliticaDemostracion', () => {
  ...
  it('mantiene coherentes los hallazgos BPM con el resultado mostrado', async () => {
    const servicio = new ServicioAnaliticaDemostracion();
    const evaluacion = await servicio.obtenerEvaluacion('eva-2026-0017');
    expect(evaluacion.items.some((item) => item.respuesta === 'IT')).toBe(true);
    expect(evaluacion.resultado.status).toBe('CALCULADO');
    if (evaluacion.resultado.status === 'CALCULADO') {
      expect(evaluacion.resultado.bpmPercentage.display).toBe('81.25');
      expect(evaluacion.resultado.riskLevel).toBe('MEDIO');
    }
  });
