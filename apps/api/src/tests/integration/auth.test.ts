...
it('devuelve el perfil mínimo real para navegación',async()=>{mocks.query.mockResolvedValueOnce({rows:[{id:'11111111-1111-4111-8111-111111111111',fullName:'Ana Empresa',roleCode:'COMPANY_ADMIN',status:'APPROVED',companyId:'22222222-2222-4222-8222-222222222222'}]});const access=await signAccessToken('11111111-1111-4111-8111-111111111111','COMPANY_ADMIN',123);const response=await request(makeApp()).get('/v1/auth/me').set('Authorization',`Bearer ${access}`);expect(response.status).toBe(200);expect(response.body.data).toEqual({id:'11111111-1111-4111-8111-111111111111',fullName:'Ana Empresa',roleCode:'COMPANY_ADMIN',status:'APPROVED',companyId:'22222222-2222-4222-8222-222222222222',authTime:123});expect(response.body.data).not.toHaveProperty('establishmentIds');expect(mocks.query.mock.calls[0][0]).toContain("c.status='ACTIVE'")});
it('omite la empresa del perfil cuando no hay membresía activa',async()=>{mocks.query.mockResolvedValueOnce({rows:[{id:'11111111-1111-4111-8111-111111111111',fullName:'Ana Empresa',roleCode:'COMPANY_ADMIN',status:'APPROVED',companyId:null}]});const access=await signAccessToken('11111111-1111-4111-8111-111111111111','COMPANY_ADMIN',123);const response=await request(makeApp()).get('/v1/auth/me').set('Authorization',`Bearer ${access}`);expect(response.status).toBe(200);expect(response.body.data.companyId).toBeNull();expect(mocks.query.mock.calls[0][0]).toContain("c.status='ACTIVE'")});
