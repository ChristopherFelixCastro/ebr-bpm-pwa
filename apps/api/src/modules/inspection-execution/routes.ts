...
router.get('/inspections/:id/location',wrap(async(q,s)=>{const p=id(q.params.id);if(!p.success)return bad(q,s,400,'VALIDATION_ERROR');ok(q,s,await service.location(p.data,actor(q)))}));
router.post('/inspections/:id/location',wrap(async(q,s)=>{const p=id(q.params.id),b=schema.locationSchema.safeParse(q.body);if(!p.success||!b.success)return bad(q,s,400,'VALIDATION_ERROR');ok(q,s,await service.recordLocation(p.data,b.data,actor(q),q.context.correlationId),201)}));
