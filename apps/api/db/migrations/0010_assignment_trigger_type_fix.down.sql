-- Local development only. Keep the cast: reverting a repair must not reintroduce it.
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    ...
    VALUES(NEW.id,NEW.case_id,(CASE WHEN prior_evaluator IS NULL THEN 'ASSIGNED' ELSE 'REASSIGNED' END)::case_assignment_event_type,prior_evaluator,NEW.evaluator_user_id,NEW.assigned_by_user_id,NEW.change_reason);
  ELSE
    INSERT INTO case_assignment_transitions(case_assignment_id,case_id,event_type,from_evaluator_user_id,to_evaluator_user_id,performed_by_user_id,reason)
