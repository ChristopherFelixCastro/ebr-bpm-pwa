...
BEGIN
  IF EXISTS (SELECT 1 FROM roles WHERE id=OLD.role_id AND code='UNIVERSAL') AND OLD.status='APPROVED' AND (NEW.status <> 'APPROVED' OR NOT EXISTS (SELECT 1 FROM roles WHERE id=NEW.role_id AND code='UNIVERSAL')) THEN
  ...
  RETURN NEW;
END; $$;
CREATE TRIGGER users_last_active_universal_guard BEFORE UPDATE OF role_id,status ON users FOR EACH ROW EXECUTE FUNCTION prevent_last_active_universal();
