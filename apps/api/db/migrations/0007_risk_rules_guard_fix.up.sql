...
BEGIN
  ...
  IF TG_OP='UPDATE' THEN
    ...
    IF NEW.status='PUBLISHED' THEN
      ...
      IF (SELECT count(*) FROM risk_factors WHERE risk_rule_version_id=NEW.id) <> 6
        OR (SELECT array_agg(code::text ORDER BY code) FROM risk_factors WHERE risk_rule_version_id=NEW.id) <> ARRAY['BPM','HACCP','INABIE','REJECTIONS','SAMPLING','VOLUME']::text[]
        OR (SELECT sum(weight) FROM risk_factors WHERE risk_rule_version_id=NEW.id) <> 1.0000 THEN RAISE EXCEPTION 'invalid factors'; END IF;
